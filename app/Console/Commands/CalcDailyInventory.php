...
class CalcDailyInventory extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            // システム日付前日までの処理
            while($targetDate < $sysDate->system_date) {
                $this->calcAndSaveDailyInventory($targetDate, $prevStockQuantity);
            }
    ...
    private function getInventory(Carbon $targetDate)
    {
        ...
        if(!$inventory) {
            ...
            if(!$inventory) {
                ...
                $targetDate = $inventory->target_date->addDay();
            }
        }
        return $inventory;
    }

    private function calcAndSaveDailyInventory(Carbon $targetDate, &$prevStockQuantity)
    {
        // 対象日付の入庫実績を取得する。
