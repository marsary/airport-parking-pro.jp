...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const loadTimeSectionWrapElem = document.getElementById('load_time_section_wrap');
  const unloadDateSectionElem = document.getElementById('unload_date_section');
  ...
  const reserveCalMonthPeriods = parseInt(document.getElementById('reserve_cal_month_periods').value) || 5;
  ...
  async function dispLoadHourTable() {
    ...
    removeTimeSelected()
    // 入庫日をAPIに送信
    const json = await apiRequest.get(BASE_PATH + "/form/calendar/load_hours",
      {load_date:loadDate}
    )
