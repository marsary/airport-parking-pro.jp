    ...
    <div class="p-user-input-auto-output__wrap l-flex--item-end">
      ...
      <div class="l-grid--col2-auto l-grid--gap2 l-flex--column--md">
        ...
        <div>
          ...
          <input type="text" id="car_number" name="car_number" maxlength="4" minlength="1" class="u-w-full-wide" value="{{old('car_number', $reserve->car_number)}}">
        </div>
        <div>
        ...
        <div>
          ...
          <input type="hidden" id="unload_time_plan" name="unload_time_plan" value="{{old('unload_time_plan', $reserve->unload_time_plan ?  $reserve->unload_time_plan->format('Y-m-d'): $reserve->unload_date_plan?->format('H:i'))}}">
        </div>
        <div>
          <label for="num_members">ご利用人数</label>
          <div class="c-form-select-color">
            <select id="num_members" name="num_members">
              @for ($i = 1; $i <= 25; $i++)
                <option value="{{ $i }}" {{ old('num_members', $reserve->num_members ?? 1) == $i ? 'selected' : '' }}>{{ $i }}名</option>
              @endfor
            </select>
          </div>
        </div>
